  ...
  \frametitle{Setting Up \& Compiling}
    \begin{itemize}
      ...
      \item Mac: https://tug.org/mactex/
        ...
        \end{itemize}
      \item I highly recommend using an editor like TeXshop, TeXworks, Vim,
        Emacs, or even Sublime Text to write LaTeX with because they all are
        either built to compile your documents or have well supported
        plugins that do it all for you.
    \end{itemize}
\end{frame}
\begin{frame}[fragile]
  \frametitle{Basic Document Structure}
    \begin{itemize}
      ...
      \item Preamble -- import external packages, set up page formatting,
        define your own macros, etc.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item There are all the normal text styles you'd expect from a word
      processor, but for some you have to use external packages.
    \item For example, to use color you need to add \verb.\usepackage{color}.
      to your preamble
  \end{itemize}
  \begin{tabular}{ll}
    \verb.\textbf{Hello World}. & \textbf{Hello World} \\ %bold-face
    \verb.\emph{Hello World}. & \emph{Hello World} \\ %emphasis
    \verb.\textsc{Hello World}. & \textsc{Hello World} \\ %all caps
    \verb.\texttt{Hello World}. & \texttt{Hello World} \\ %typewriter style
...
\begin{frame}[fragile]
  \frametitle{Titles}
  \begin{itemize}
    \item There are some shortcuts for you to make nice titles,
      but you don't \emph{have} to use them
...
\begin{frame}[fragile]
  \frametitle{Sections}
  \begin{itemize}
    \item There are some shortcuts for you to make nice section headings
      (think chapters in a book, questions on a problem set, etc.)
...
\begin{frame}[fragile]
  \frametitle{More Alignment (Advanced)}
  \begin{itemize}
...
\begin{frame}[fragile]
  \frametitle{Mid-point review - math mode}
  How can we make..\\
  \begin{itemize}
    \item Let set $S = \{1, 2, \cdots, 3\}$
    \item Let $x = a^{100}_1 + a^{100}_2$
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Let set $S = \{1, 2, \cdots, 30\}$

  \begin{verbatim}
    Let set $S = \{1, 2, \cdots, 30\}$
  \end{verbatim}

  Let $x = a^{100}_1 + a^{100}_2$
  \begin{verbatim}
    Let $x = a^{100}_1 + a^{100}_2$
  \end{verbatim}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Mid-point review - alignment}

  How do we make...\\

  \begin{align*}
    y = f(x) &= x + x + x \text{ (} \because \text{idk)}\\
             &= 3x
  \end{align*}
\end{frame}
...
\begin{frame}[fragile]
  ...
  %use xparse, xargs package for more default, optional values
  \begin{itemize}
    \item Define environments using \verb.\newcommand{\your_command_name}{what it does}.
    \item \verb+\newcommand{\name}[num_args][default_arg]{what..}+
...
\begin{frame}[fragile]
  ...
  We can also overwrite existing commands (or ones that we made) using
  \verb.\renewcommand. with the same syntax as \verb.\newcommand.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item simple box around an equation
  \end{itemize}
  \begin{multicols}{2}
    \begin{verbatim}
      \begin{equation}
  ...
  \begin{itemize}
    \item For more capabilities, use empheq package
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Figures \& Graphics}
  \begin{itemize}
    \item include path and extensions in preamble for ease
  \end{itemize}
  \begin{verbatim}
  ...
  \end{verbatim}
  \begin{itemize}
    \item Then you can just put
  \end{itemize}
  \begin{verbatim}
    \includegraphics[width=8cm]{awesomesauce}
  \end{verbatim}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Verbatim}
  \begin{itemize}
    \item Write things within verbatim
  \end{itemize}
  \begin{verbatim}
    \begin{verbatim.}
  ...
  \end{verbatim}
  \begin{itemize}
    \item For sytax highlighting and better codes, use listings package
...
\begin{frame}[fragile]
  \frametitle{Verbatim}
  \begin{itemize}
    \item for short verbatim words, use \verb.\verb.
    \item \verb.\verb. turns words that are within the two separators into
      verbatim.
    \item Separater is the letter following the \verb.\verb.
    \begin{verbatim}
      \verb+Everything Here is Verbatim+
    \end{verbatim}
  \end{itemize}
\end{frame}
%----------------------------------------------------------------------------------------
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item reference to Equation \ref{pythagorean}
    \item reference to Equation \verb+\ref{pythagorean}+
  \end{itemize}
\end{frame}
